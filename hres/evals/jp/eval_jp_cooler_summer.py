    ...
    for day in tqdm(range(1, 32)):
        ...
        for fh in range(24, 337, 24):
            ...
            if fh <= 240:
                ...
                hres_temp = grbs.select(name="2 metre temperature")[0].values[:720,:] - 273.15
                hres_interp = RegularGridInterpolator((wm_lats, wm_lons), hres_temp, method="linear")
            for city, (lat, lon) in jp_cities.items():
                temp_era5 = era5_interp([lat, lon])[0] if use_era5 else np.nan
                ...
                # print(f"{city}: era5: {temp_era5:.3f} wm: {temp_wm:.3f}  pointy: {temp_pointy}")
                # city_errors[city].append((init_datestr, fh, temp_pointy, temp_era5, temp_wm))

        with open("/fast/wbhaoxing/deep/evals/cities/jp_pointy_era5_wm_hres_summer.json", "w") as f:
