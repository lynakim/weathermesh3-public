...
days = np.arange(len(gc_error_weighted_mean.isel(prediction_timedelta=slice(1, -1)))) / 4

# Create the plot
plt.figure(figsize=(12, 6))

# Mean and envelope boundaries (mean ± 1 standard deviation)
mean_values = gc_error_weighted_mean.isel(prediction_timedelta=slice(1, -1)).values
std_values = gc_error_weighted_std.isel(prediction_timedelta=slice(1, -1)).values
...
plt.savefig('mean_continuity_error_with_envelope_over_jan_01_weighted.png', dpi=300, bbox_inches='tight')
plt.show()