...
@launch(nodes={'glass': 5} ,port=29506, start_method="spawn",clear_cache=False)#,kill_nvidia=False, zulip=True, ping='@**Joan Creus-Costa**',validate=False)
def Jan23_johnrefactor_4x32():
    config.nope = 0
    ...
    MicrowaveEncoder = PointObPrEncoder
    ...
    model = DAOnlyModel(
        conf,
        anl_encoders=[],
        obs_encoders=[MicrowaveEncoder(mesh_1bamua, conf, out_dim=O), MicrowaveEncoder(mesh_atms, conf, out_dim=O), PointObPrEncoder(igra_mesh, conf, background='full')],
...
@launch(nodes={'baga': 5} ,port=29506, start_method="spawn",clear_cache=False)#,kill_nvidia=False, zulip=True, ping='@**Joan Creus-Costa**',validate=False)
def Jan23_johnrefactor():
    config.nope = 0
    tdates = get_dates([(D(1979, 1, 23), D(2024, 2, 1))])
    extra_output = ['15_msnswrf', '45_tcc', '168_2d', '246_100u', '247_100v', '142_lsp', '143_cp', '201_mx2t', '202_mn2t', 
             '142_lsp-6h', '143_cp-6h', '201_mx2t-6h', '202_mn2t-6h']
...
@launch(nodes={'muir': 5} ,port=29506, start_method="spawn",clear_cache=False,kill_nvidia=False, zulip=True, ping='@**Joan Creus-Costa**',validate=False)
def Jan20_moreobs_sfc():
    config.nope = 1
    tdates = get_dates([(D(1979, 1, 23), D(2024, 2, 1))])
    extra_output = ['15_msnswrf', '45_tcc', '168_2d', '246_100u', '247_100v', '142_lsp', '143_cp', '201_mx2t', '202_mn2t', 
             '142_lsp-6h', '143_cp-6h', '201_mx2t-6h', '202_mn2t-6h']
    ...
    from model_latlon.da_encoders import MicrowaveData, BalloonData, SurfaceData
    mesh_1bamua = MicrowaveData("1bamua") # atms,1bamua
    mesh_atms = MicrowaveData("atms") # atms,1bamua
    from datasets import AnalysisDataset, IgraDataset, MicrowaveDataset, SurfaceDataset
    igra_mesh = BalloonData()
    ...
    model = DAOnlyModel(
        conf,
        anl_encoders=[],
        obs_encoders=[MicrowaveEncoder(mesh_1bamua, conf, background='full'), MicrowaveEncoder(mesh_atms, conf, background='full'), PointObPrEncoder(igra_mesh, conf, background='full'), PointObSfcEncoder(mesh_sfc, conf, background=None)],
        da_transformer= DATransformer(conf),
        decoders=[SimpleConvDecoder(omesh, conf)]
        )
    print(model)
...
@launch(nodes={'baga': 5} ,port=29506, start_method="spawn",clear_cache=False,kill_nvidia=False, zulip=True, ping='@**Joan Creus-Costa**',validate=False)
def Jan14_moreobs_johnembed_transposed():
    config.nope = 0
    tdates = get_dates([(D(1979, 1, 23), D(2024, 2, 1))])
    extra_output = ['15_msnswrf', '45_tcc', '168_2d', '246_100u', '247_100v', '142_lsp', '143_cp', '201_mx2t', '202_mn2t', 
             '142_lsp-6h', '143_cp-6h', '201_mx2t-6h', '202_mn2t-6h']
