...
def eval_loop(model, data, eval_path, config, eval_every, s2s=False):
    ...
    dataloader = DataLoader(data, batch_size=1, shuffle=False, num_workers=10, collate_fn=collate_fn)
    ...
    for i in range(n_dates):
        ...
        print("model run took", time.time()-t0)
        ...
        for j,(ddt,y) in enumerate(ys):
            ...
            bylat_out = f'{error_dir}/{get_date_str(nix)}+{ddt}.bylat.pickle'
            t0 = time.time()
            bylatdic = {}
            with torch.autocast(enabled=False, device_type='cuda', dtype=torch.float32):
                for error_name,metric_fn in all_metric_fns.items():
                    ...
                    metrics, bylat = compute_errors(y,yt,weights, output_mesh, metric_fn=metric_fn, only_sfc=s2s, resolution=resolution, bylat=True)
                    bylatdic[error_name] = bylat
                    out[error_name] = metrics
            print("error comp", time.time()-t0)
            t0 = time.time()
            with open(eval_out,'w') as f:
                json.dump(out,f,indent=2)
            with open(bylat_out,'wb') as f:
                pickle.dump(bylatdic,f)
...
if __name__ == '__main__':
    ...
    #modelfn = get_fenrir
    #modelfn = get_graphbs_tinyyolo
    modelfn = get_graphbs_yolo
    #modelfn = get_joansucks
    # default args
    is_s2s = False
    ...
    proc_spec = ForecastStep3D.simple_gen_todo(list(range(0,24*5+1,24)), [6])
    ...
    EvalModel(modelfn, requested_dates, input_datasets, output_datasets, EvalConfig(only_at_z=[0,12], model_proc_spec = proc_spec, model_name = modelfn.__name__[4:], save_every=None), s2s=is_s2s) #, eval_every=3)
