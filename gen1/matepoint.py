...
def matepoint_pipeline(ctx,stream):
    ...
    if len(ctx) >= 1: 
        with torch.cuda.stream(stream):
            ctx[-1][0] = [x.to(d, non_blocking=True) if hasattr(x, "device") else x for x, d in zip(*ctx[-1])]
    ...
    **kwargs
    ...
    def recompute_fn(*inputs):
        ...
        with torch.random.fork_rng(
            ...
            with device_autocast_ctx, torch.cpu.amp.autocast(**cpu_autocast_kwargs), \
                 recompute_context:
                #print("hiya all contexts depth", len(all_contexts))
                #all_contexts.pop()
                #print('yooo',len(matepoint_ctx))
                args, devices = matepoint_ctx.pop()
                with torch.cuda.stream(stream):
                ...
                #print('gpu bw', len(matepoint_ctx), elist(args))
    ...
    with torch.cuda.stream(stream):
        ...
        for arg in args:
            ...
            #torch.cuda.synchronize()
            #print('gpu',elist([arg]))
            #print('cpu',elist([xcpu]))
            args_cpu.append(xcpu)
    #torch.cuda.stream(stream).synchronize()
    #stream.synchronize()
    #print('gpu fw',elist(args))
    #print('cpu',elist(args_cpu))
    new_frame.input_saver = _NoopSaveInputs.apply(dummy, kwargs, len(matepoint_ctx))
    matepoint_ctx.append([args_cpu, devices])
