...
class SimpleConvEncoder(nn.Module):
    def __init__(self, mesh, config):
        ...
        self.conv_pr = nn.Conv3d(in_channels=self.mesh.n_pr_vars, out_channels=c.latent_size, kernel_size=c.patch_size, stride=c.patch_size)
        self.conv_sfc = nn.Conv2d(in_channels=self.total_sfc, out_channels=c.latent_size, kernel_size=c.patch_size[1:], stride=c.patch_size[1:])
        self.tr = SlideLayers3D(dim=c.latent_size, depth=c.encdec_tr_depth, num_heads=c.num_heads, window_size=c.window_size, 
            harebrained=c.harebrained, embedding_module=c.embedding_module, checkpoint_type=c.checkpoint_type)
    ...
    def forward(self, x, t0s):
        ...
        x_sfc = x[:,:,:,self.mesh.n_pr:].permute(0,3,1,2) # x_sfc is shape B,C,H,W for convs
        
        # Handles nan inputs
        if torch.isnan(x_sfc).any():
            x_sfc = torch.nan_to_num(x_sfc, nan=0.0)
        
        addl = get_additional_vars(t0s).permute(0,3,1,2)
        x_sfc = torch.cat((x_sfc,addl,self.const_data),dim=1)
        x_sfc = self.conv_sfc(x_sfc)

        x_pr = x[:,:,:,:self.mesh.n_pr].view(B,H,W,Cpr,D).permute(0,3,4,1,2) # x_pr is shape B,C,D,H,W for convs
        x_pr = self.conv_pr(x_pr)

        x = torch.concat((x_pr.permute(0,2,3,4,1), x_sfc.permute(0,2,3,1)[:,None]),dim=1)

        x = add_posemb(x)
        x = tr_pad(x, self.config.window_size)
...
class ResConvEncoder(nn.Module):
    ...
    def embedding_crap(self, x_pr, x_sfc):
        ...
        if self.config.tr_embedding == 'sincos':
            ...
            x = x + pe
        if self.config.harebrained:
            x = self.to_hb(x) 
            assert False, "figure out padding shit"
        else:
            _, _, _, W, _ = x.shape
