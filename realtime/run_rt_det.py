...
# default forecast times: hourly for 5 days, then 6-hourly out to 15 days
DTS = list(range(0, 24*5, 1)) + list(range(24*5, 24*15+1, 6))
...
def load_model():
    ...
    # from gen1.package import get_hegelcasioquad # till 2024-09-23
    # rtyamahabachelor3 till 2024-10-03
    
    getter = globals().get(f"get_{DEFAULT_MODEL_NAME}")
    if getter is None:
        raise ValueError(f"get_{DEFAULT_MODEL_NAME}() not found in evals.package_neo")
    model = getter()
...
def check_processed(date):
    for dt in DTS:
        output_path = f'{OUTPUT_PATH}/{get_output_filepath(date, dt)}.*.npy'
        if not glob.glob(output_path):
            return False
    print(f"Already processed all timesteps in {DTS}", flush=True)
    return True
    ...
    #     output_path = f'{OUTPUT_PATH}/{get_output_filepath(date, hr)}.*.npy'
    #     if glob.glob(output_path):
    #         processed_count += 1

    # if processed_count > 0:
    #     print(f"Already processed {processed_count} of {nsteps} forecast hours", flush=True)
...
def save_output(out, h, date):
    ...
    output_fn = save_instance(out, f'{OUTPUT_PATH}/{get_output_filepath(date, round(h))}', model.config.output_mesh, DEFAULT_MODEL_NAME, upload_metadata=S3_UPLOAD, )[len(OUTPUT_PATH)+1:]
...
def upload_output(output_fn, h):
    ...
    start_time = time.time()
    s3_fn = f'{S3_OUTPUT_PATH}/{output_fn}'.replace('+', '/det/', 1)
...
def run_if_needed(idempotent=True, forecast_zero=None):
    ...
    if check_processed(forecast_zero) and idempotent:
        ...
        print("Not processed, running", flush=True)

    load_model()
    if S3_UPLOAD:
        upload_thread = threading.Thread(target=upload_worker)
        upload_thread.start()
        run_and_save(forecast_zero)
...
def main():
    print(f"Started process {os.getpid()} for run_rt_det", flush=True)
    ...
    if args.output:
        ...
        OUTPUT_PATH = f"/fast/realtime/outputs/{args.output}"
        S3_OUTPUT_PATH = f"{args.output}"
    ...
    run_if_needed(forecast_zero=args.forecast_zero, idempotent=args.idempotent)
