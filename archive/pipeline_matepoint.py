...
def _checkpoint_without_reentrant_generator(
    ...
    **kwargs
    ...
    def recompute_fn(*inputs):
        ...
        with torch.random.fork_rng(
            ...
            with device_autocast_ctx, torch.cpu.amp.autocast(**cpu_autocast_kwargs), \
                 recompute_context:
                #print("hiya all contexts depth", len(all_contexts), args)
                args, devices = all_contexts.pop()
                print(len(all_contexts), "huh devices", [x.device for x in args if hasattr(x, "device")])
                """
                print("popped", args, devices)
                print("huh devices", [x.device for x in args if hasattr(x, "device")])
                print("previous all_contexts", all_contexts[-1])
                for x, d in all_contexts[-1]:
                    print("aaaa", x, d, "hasattr", hasattr(x, "device"))
                """
                if len(all_contexts) >= 1: all_contexts[-1][0] = [x.to(d, non_blocking=True) if hasattr(x, "device") else x for x, d in zip(*all_contexts[-1])] # pipeline
                #if len(all_contexts) >= 2: all_contexts[-2][0] = [x.to(d, non_blocking=True) if hasattr(x, "device") else x for x, d in zip(*all_contexts[-2])] # pipeline
                #print("neo all_contexts", all_contexts[-1])
                fn(*[x.to(d, non_blocking=True) if hasattr(x, "to") else x for x, d in zip(args, devices)], **kwargs)
    ...
    devices = [x.device if hasattr(x, "device") else None for x in args]
    args = [x.to(cpu, non_blocking=True) if hasattr(x, "to") else x for x in args]

    new_frame.input_saver = _NoopSaveInputs.apply(dummy, kwargs, len(all_contexts))#*[x.to(cpu, non_blocking=True) if hasattr(x,"to") else x for x in args])
    all_contexts.append([args, devices])
