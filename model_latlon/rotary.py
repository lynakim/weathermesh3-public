...
def apply_liere_pos_emb(rotations, t):
    return einops_einsum(t, rotations, 'b h n d, n d e -> b h n e')
...
class Liere(Module):
    ...
    def forward(
        ...
        rots = []
        for i in range(N//cs):
            #rots.append(torch.matrix_exp(matrices[cs*i:cs*(i+1)].float()))
            rots.append(torch.matrix_exp(matrices[cs*i:cs*(i+1)]).float())
        rotations = torch.cat(rots, axis=0)

        #rotations = torch.matrix_exp(matrices.float())
        return rotations
...
class RotaryEmbedding(Module):
    def __init__(
        ...
        #self.freqs = nn.Parameter(freqs.detach(), requires_grad = learned_freq) # gives runtime error if requires_grad is False https://chat.windbornesystems.com/#narrow/stream/201-tech-dlnwp/topic/s2s.3A.20runs/near/6698666
        assert not learned_freq, "you probably want to change freqs from buffer back to Parameter, to use learned freqs"
        self.register_buffer('freqs', freqs.detach(), persistent = False)
